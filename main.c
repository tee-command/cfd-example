...
void solve_p(double p[NX][NY], const double u_star_x[NX][NY],
  ...
  // 周期境界のみの場合、圧力は定数分の不定性を持つので基準点を固定
  const double p_ref = p[X_MIN][Y_MIN];
  for (int y = Y_MIN; y <= Y_MAX; ++y) {
    for (int x = X_MIN; x <= X_MAX; ++x) {
      p[x][y] -= p_ref;
    }
...
void update_fields(double u[NX][NY], double v[NX][NY], double p[NX][NY],
                   ...
                   double advection_y_old[NX][NY], int *is_first_step) {
  // 時刻nの移流項
  double advection_x[NX][NY];
  double advection_y[NX][NY];
  solve_advection(advection_x, advection_y, u, v);

  // 初回ステップは advection^{-1} が存在しないため advection^0 で代用
  // → 係数が (1.5, -0.5) から実質 (1.0, 0.0) になり、初回だけ1次精度Eulerで開始
  if (*is_first_step) {
    for (int y = Y_MIN; y <= Y_MAX; ++y) {
      for (int x = X_MIN; x <= X_MAX; ++x) {
...
int main() {
  ...
  while (1) {
    ...
    // 時間進行
    update_fields(u, v, p, advection_x_old, advection_y_old, &is_first_step);
    step++;
  }
